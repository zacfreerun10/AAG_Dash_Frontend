...
export default function HospitalMap({ hospitals, boundary }) {
  ...
  return (
    ...
    {/* Summary card of hospitals */}
    {hospitals.length > 0 && (
        <div className="chart-container">
          <h3 style={{ fontSize: "16px", fontWeight: "bold", marginBottom: "8px" }}>Hospitals ({hospitals.length})</h3>
          <ul style={{ listStyle: "none", padding: 0, maxHeight: "200px", overflowY: "auto" }}>
            {hospitals.map((h, index) => (
              <li key={index} style={{ marginBottom: "6px", fontSize: "14px" }}>
                🏥 {h.properties.LandmkName || "Unnamed Hospital"}
              </li>
            ))}
          </ul>
        </div>
